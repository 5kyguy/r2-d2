...
description: >
  ...
  Covers reporting a confirmed R2-D2 bug upstream — see reporting.md.
---
...
```bash
r2-d2-crash-mute '<program>'        # silence it
r2-d2-crash-mute '<program>' off    # let it speak again
r2-d2-crash-mute                    # list what is muted
```
...
A diagnosis run by hand from `r2-d2-k2so-crash <pid>` has neither, so take
...
## If it is an R2-D2 bug

Most application crashes are upstream bugs in those applications, not R2-D2's
doing. In the minority of cases where the cause really does sit within R2-D2's
